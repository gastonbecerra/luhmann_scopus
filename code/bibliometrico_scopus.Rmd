...
acc <- autores2 %>% count(id,uni) %>% 
  ...
  mutate( acc=case_when(
    !is.na(academy) & !is.na(private) ~ "Academia + Corporate",
...
acc %>% 
  ...
  ggplot(aes(x=year,y=n,fill=acc)) +
    ...
    scale_fill_jama(name = "Autorship type") +
    theme_minimal() +
    scale_x_continuous(name="Years", 
                     breaks = c(2010:2020), 
                     labels = c(2010:2020),
                     minor_breaks = NULL) +
  labs(caption="Figure 3. Distribution of authorship type") +
  theme(axis.title.y=element_blank()) +
  theme(plot.caption = element_text(hjust=0.5, size=rel(1))) 
...
autores %>% 
  ...
  theme_graph() +
  theme(legend.position = "none")+
  labs(caption="Figure 4. Collaboration by country (countries with n > 50)") +
  theme(plot.caption = element_text(hjust=0.5, size=rel(1)))
...
```{r topcitedjournals, eval=FALSE, fig.height=4, fig.width=8, message=FALSE, warning=FALSE, include=FALSE}
