...
In terms of collaboration (according by co-joint authorship), we see USA as the most central country, with the most number of collaborations, closely linked to China, United Kingdom, Canada, and Germany (fig. 5).
...
autores %>% 
  ...
  geom_node_point(aes(size = art_totales, fill=art_totales), shape=21) +
...
TENGO QUE CORREGIR LOS NOMBRE DE LOS ROWS
...
year_ave <- autores %>% 
  left_join(articles %>% select(id,year)) %>% filter(year>=2010) %>%
  group_by(year,id) %>% summarise(autores=n()) %>%
  group_by(year) %>% summarise(ave=mean(autores),sd=sd(autores)) 

subra1 <- subra %>% filter(autores<11)
subra2 <- subra %>% filter(autores>10) %>% colSums(., na.rm = TRUE)
subra2[1]$autores <- "+10"

# average
subra3 <- unlist(year_ave$ave)
subra3 <- c(NA, subra3)

# sd
subra4 <- unlist(year_ave$sd)
subra4 <- c(NA, subra4)

Ns <- colSums(subra[which(subra$autores==1),-1], na.rm = TRUE)
Nm <- colSums(subra[which(subra$autores>1),-1], na.rm = TRUE)
subra5 <- c(NA, Nm / (Nm+Ns))

rbind(subra1,subra2,subra3,subra4,subra5) %>% as.data.frame() %>% 
  gt() %>% 
  fmt_number(columns = 2:12, rows = 1:11, decimals = 0) %>%
  fmt_number(columns = 2:12, rows = 12:14, decimals = 2) %>%
  fmt_missing(columns = 2:11, missing_text = "-") %>%
  fmt_missing(columns = 1, rows = 12, missing_text = "Average") %>%
  fmt_missing(columns = 1, rows = 13, missing_text = "SD") %>%
  fmt_missing(columns = 1, rows = 14, missing_text = "SI") %>%
  tab_header(
    title = "Number of authors by year"
  ) %>%
  tab_options(
    table.font.size = "11px"
  ) %>%
